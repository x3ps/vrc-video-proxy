...
func TestUpstreamClientTuning(t *testing.T) {
	...
	if tr.MaxIdleConnsPerHost <= 2 {
		t.Fatalf("MaxIdleConnsPerHost = %d, want > 2 (avoid seek churn)", tr.MaxIdleConnsPerHost)
	}
...
func TestUpstreamClientProxy(t *testing.T) {
	proxyURL, _ := url.Parse("socks5://127.0.0.1:1080")
	tr := NewUpstreamClient(proxyURL).Transport.(*http.Transport)
	if tr.Proxy == nil {
	...
	def := NewUpstreamClient(nil).Transport.(*http.Transport)
	if def.Proxy == nil {
